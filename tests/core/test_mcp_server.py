...
async def test_mcp_exposes_memory_toolcall_roundtrip(tmp_path) -> None:
    ...
    assert {"memory.write", "context.get", "session.raw_tail", "project.info"}.isdisjoint(tool_names)
    ...
    assert tools["memory_write"].annotations.destructiveHint is False
    assert tools["memory_write"].annotations.readOnlyHint is False
    assert tools["memory_write"].annotations.idempotentHint is True
...
async def test_mcp_context_get_reads_recent_session_and_memory(tmp_path) -> None:
    mcp = create_mcp(tmp_path / "state.sqlite", None)
    await mcp.call_tool("memory_write", {"summary": "记忆模块已经接入 MCP", "scope": "project:ctx-test"})

    _, ctx = await mcp.call_tool("context_get", {"query": "MCP 记忆模块", "scope": "project:ctx-test"})
    assert ctx["ok"]
